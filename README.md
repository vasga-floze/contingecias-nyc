...
--POR DOCUMENTO (TICKET/FACTURA)
SELECT FECHA_ORDEN 'FECHA EMISIÓN', DOC_FISCAL 'RESOLUCION', REPLACE(CLAVE_REFERENCIA_DE,'-','') 'SERIE',
	REPLACE(CLAVE_DE,'-','') 'NÚMERO DE DOCUMENTO (DEL)', REPLACE(CLAVE_DE,'-','') 'NÚMERO DE DOCUMENTO (AL)',
	TOTAL_FACTURA 'VENTAS GRAVADAS LOCALES' 
FROM  CANNYSHOP.FACTURA 
WHERE FECHA BETWEEN '20230801' AND '20230831'
ORDER BY FECHA,USUARIO,DOC_FISCAL;
...
A este script solo se le debe cambiar la fecha, adaptar el rango de fechas que se desean generar:
```
USE EXIMP600;

SELECT
    CASE
        WHEN LEFT(consny.CAJA_POS.BODEGA, 1) = 'C' THEN 'CARISMA'
        WHEN LEFT(consny.CAJA_POS.BODEGA, 1) = 'E' THEN 'EVER'
        WHEN LEFT(consny.CAJA_POS.BODEGA, 1) = 'N' THEN 'NERY'
        ELSE LEFT(consny.CAJA_POS.BODEGA, 1) 
    END AS EMPRESA,
    CONVERT(DATE, consny.DOCUMENTO_POS.FCH_HORA_COBRO) AS FECHA,
    consny.DOCUMENTO_POS.DOCUMENTO,
    consny.DOCUMENTO_POS.CLIENTE,
    consny.DOCUMENTO_POS.NOMBRE_CLIENTE,
    CASE
        WHEN consny.documento_pos.tipo = 'F' THEN consny.DOCUMENTO_POS.TOTAL_PAGAR
        ELSE consny.DOCUMENTO_POS.TOTAL_PAGAR * -1
    END TOTAL_PAGAR,
    CASE
        WHEN consny.DOCUMENTO_POS.DOCUMENTO LIKE '%TIQ%' THEN 'TICKET'
        WHEN consny.DOCUMENTO_POS.DOCUMENTO LIKE '%CCF%' THEN 'CREDITO FISCAL'
        WHEN consny.DOCUMENTO_POS.DOCUMENTO LIKE '%FAC%' THEN 'FACTURA'
    END TIPO_DOCUMENTO,
    consny.FORMA_PAGO.DESCRIPCION FORMA_PAGO,
    consny.BODEGA.NOMBRE TIENDA
FROM
    consny.DOCUMENTO_POS
INNER JOIN
    consny.PAGO_POS ON consny.DOCUMENTO_POS.DOCUMENTO = consny.PAGO_POS.DOCUMENTO
        AND consny.DOCUMENTO_POS.TIPO = consny.PAGO_POS.TIPO
        AND consny.DOCUMENTO_POS.CAJA = consny.PAGO_POS.CAJA
INNER JOIN
    consny.FORMA_PAGO ON consny.PAGO_POS.FORMA_PAGO = consny.FORMA_PAGO.FORMA_PAGO
INNER JOIN
    consny.CAJA_POS ON consny.DOCUMENTO_POS.CAJA = consny.CAJA_POS.CAJA
INNER JOIN
    consny.BODEGA ON consny.CAJA_POS.BODEGA = consny.BODEGA.BODEGA
WHERE
    (consny.DOCUMENTO_POS.FCH_HORA_COBRO BETWEEN '2023-08-01 00:00' AND '2023-09-16 23:59')
ORDER BY
    2, 9, 3;
```
